...
// Raised from 0.004 after measuring this machine: with the room quiet the floor
// sits at 0.0004-0.0009 and ambient peaks reach 0.0023, so a bar of 0.004 was
// under 2x the loudest thing the empty room produces - close enough that a fan,
// a chair or a keyboard cleared it and was sent as speech. Real speech measured
// 0.032-0.064 in the same session, twenty times higher, so there is a wide gap
// to sit in. 0.012 is ~5x ambient peak and ~3x below the quietest speech seen.
//
// The trade is deliberate and it does cut both ways: raise this too far and the
// gate stops opening for someone speaking quietly, which is the failure this
// number was lowered to fix once before. Hold the spacebar to force a turn, and
// lower it live without a rebuild if it is too deaf in your room:
//
//   localStorage.setItem('ben_min_open_rms', '0.008'); location.reload();
const MIN_OPEN_RMS = tunable('min_open_rms', 0.012);
// Ratio against the measured floor, for rooms louder than this one. Raised with
// the absolute bar so a noisy room does not simply float the floor up under it.
const OPEN_FACTOR = tunable('open_factor', 4);
...
// Half the open bar, keeping the hysteresis ratio it had at the old values.
const MIN_CLOSE_RMS = tunable('min_close_rms', 0.006);
